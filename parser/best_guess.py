...
def main():
    ...
    first_pass_guess = first_pass_sort[0][1]
    after_dvbs2_guess = dvbs2_rankings[0][1]
    after_after_dvbs2_guess = ip_rankings[0][1]
    
    if isinstance(first_pass_guess, MpegtsParser):
        subprocess.call(["python3", "-m", "parser.parsers.mpegts.mpegts_parser", capture_file])
    elif isinstance(first_pass_guess, DVBS2Parser):
        if isinstance(after_dvbs2_guess, IPv4Parser):
            subprocess.call(["python3", "-m", "parser.dvbs2_ip", capture_file])
        elif isinstance(after_dvbs2_guess, Len2SplitCacheGSEParser): 
            subprocess.call(["python3", "-m", "parser.dvbs2_gse", "--parser", "len2.split", capture_file])
            pass
        elif isinstance(after_dvbs2_guess, StandardLenSplitCacheGSEParser):
            subprocess.call(["python3", "-m", "parser.dvbs2_gse", "--parser", "stdlen.split", capture_file])
            pass
        elif isinstance(after_dvbs2_guess, StandardLenStandardCacheGSEParser):
            subprocess.call(["python3", "-m", "parser.dvbs2_gse", "--parser", "stdlen.std", capture_file])
            pass
        elif isinstance(after_dvbs2_guess, Len2StandardCacheGSEParser): 
            subprocess.call(["python3", "-m", "parser.dvbs2_gse", "--parser", "len2.std", capture_file])
            pass
